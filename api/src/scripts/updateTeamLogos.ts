 ...
 */
function normalizeTeamNames(name: string): string[] {
  const variants = [name];
  ...
  // Variantes connues
  if (/juventus/i.test(name)) variants.push("Juventus");
  if (/inter/i.test(name)) variants.push("Inter Milan");
  if (/bayern/i.test(name)) variants.push("Bayern");
  if (/dortmund/i.test(name)) variants.push("Borussia Dortmund");
  if (/salzburg/i.test(name)) variants.push("Salzburg");
  if (/paris/i.test(name)) variants.push("Paris Saint-Germain");
  if (/chelsea/i.test(name)) variants.push("Chelsea");
  if (/barcelona/i.test(name)) variants.push("Barcelona");
  if (/madrid/i.test(name)) variants.push("Real Madrid");
 ...
 */
async function getTeamLogoByNameSmart(teamName: string) {
  ...
  for (const name of namesToTry) {
    try {
      ...
      console.error(
        `Erreur lors de la récupération du logo pour ${name}:`,
        error
      );
    }
  }
...
async function getTeamLogoByName(teamName: string) {
  try {
    ...
    console.error(
      `Erreur lors de la récupération du logo pour ${teamName}:`,
      error
    );
    return null;
  }
  ...
  try {
    ...
    for (const competitor of competitors) {
      ...
      if (!teamInfo && competitor.short_name) {
        console.log(
          `Tentative avec le short_name pour ${competitor.short_name}...`
        );
      ...
      if (teamInfo && teamInfo.logo) {
        ...
        updatedCount++;
        console.log(
          `Logo mis à jour pour ${competitor.name}: ${teamInfo.logo}`
        );
      } else {
        console.log(
          `Aucun logo trouvé pour ${competitor.name} / ${competitor.short_name}`
        );
      }
    ...
    console.log(
      `Mise à jour terminée. ${updatedCount}/${competitors.length} logos mis à jour.`
    );
  } catch (error) {
