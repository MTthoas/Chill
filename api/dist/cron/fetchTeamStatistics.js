    ...
    for (const season of seasons) {
        for (const competitor of season.competitors) {
            try {
                ...
                if (existingStats) {
                    console.log(`Statistiques déjà présentes pour competitor ${competitor.special_id} saison ${season.special_id} aujourd'hui, on skip.`);
                    continue;
                }
                // Appel à l'API SportRadar pour récupérer les stats de l'équipe
                const statisticsResponse = await fetch(`https://api.sportradar.com/soccer/trial/v4/en/seasons/sr%3Aseason%3A${season.special_id}/competitors/sr%3Acompetitor%3A${competitor.special_id}/statistics.json`, {
                    method: "GET",
                    headers: {
                ...
                if (!statisticsResponse.ok) {
                    console.error(`Erreur SportRadar pour competitor ${competitor.special_id} saison ${season.special_id}: ${statisticsResponse.status}`);
                    continue;
                }
                ...
                console.log(`Statistiques mises à jour pour competitor ${competitor.special_id} saison ${season.special_id}`);
            }
            catch (error) {
                console.error(`Erreur lors de la récupération des stats pour competitor ${competitor.special_id} saison ${season.special_id}:`, error);
            }
        }
    }
        ...
        for (const competitor of season.competitors) {
            try {
                ...
                if (nextMatch) {
                    ...
                    nextMatchInfo = `Prochain match: vs ${opponent?.name || "inconnu"} le ${new Date(nextMatch.start_time).toLocaleDateString()}`;
                }
                ...
                const statsText = competitor.statistics
                    ...
                    .join(", ");
                if (!statsText) {
                    console.log(`Pas de statistiques pour ${competitor.name}, skip avis IA`);
                    continue;
                }
                ...
                if (nextMatchInfo && nextMatch) {
                    ...
                    if (opponent) {
                        const opponentFull = season.competitors.find((c) => c.id === opponent.id);
                        if (opponentFull &&
                    ...
                    prompt = `You are a professional sports analyst. Here are the stats for ${competitor.name}: ${statsText}.\nNext match: vs ${opponent?.name || "unknown"} (stats: ${opponentStatsText}).\nCompare both teams, analyze strengths, weaknesses, and context. Conclude with a clear investment order for ${competitor.name}:\nOrder: [buy/sell]\nOnly answer with the analysis and finish with 'Order: buy' or 'Order: sell' on a separate line. No other text after the order.`;
                }
                else {
                ...
                if (!chatGptResponse.ok) {
                    console.error(`Erreur ChatGPT pour ${competitor.name}: ${chatGptResponse.status}`);
                    continue;
                }
                const chatGptData = await chatGptResponse.json();
                const rawContent = chatGptData.choices[0]?.message?.content?.trim() ||
                ...
                if (orderMatch) {
                    ...
                    if (lastLine === "buy")
                        order = "buy";
                    else if (lastLine === "sell")
                        order = "sell";
                    else if (/buy/i.test(rawContent))
                        order = "buy";
                    else if (/sell/i.test(rawContent))
                        order = "sell";
                    else {
                        // fallback explicite : si pas trouvé, on log la réponse brute et on force 'sell'
                        console.warn(`Aucun order détecté dans l'avis IA pour ${competitor.name} (special_id: ${competitor.special_id}), réponse brute : ${rawContent} -- fallback sur 'sell'`);
                        order = "sell";
                    }
                }
                ...
                if (order === "buy" || order === "sell") {
                    ...
                    console.log(`Avis généré pour ${competitor.name} (special_id: ${competitor.special_id}): ${advice} [order: ${order}]`);
                }
                else {
                    console.warn(`Aucun order valide pour ${competitor.name}, avis non sauvegardé.`);
                }
                // Pause pour éviter le rate limit OpenAI
                await new Promise((resolve) => setTimeout(resolve, 1000));
            }
            catch (error) {
                console.error(`Erreur lors de la génération d'avis pour ${competitor.name}:`, error);
            }
        }
    }
}
